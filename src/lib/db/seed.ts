...
async function main() {
  ...
  // Apply English translations from pre-exported JSON files
...
/**
 * Apply pre-exported English translations from public/translations/ JSON files.
 * These files are generated by scripts/export-translations.mjs.
 * If files don't exist, translations are skipped (they can be generated later
 * via scripts/translate-content.mjs using the OpenRouter API).
 */
async function applyTranslations() {
  const translationsDir = resolve(PUBLIC_DIR, "translations");
  const versionsFile = resolve(translationsDir, "versions-en.json");
  const unitsFile = resolve(translationsDir, "units-en.json");
  const articlesFile = resolve(translationsDir, "articles-en.json");

  if (!existsSync(versionsFile) || !existsSync(unitsFile) || !existsSync(articlesFile)) {
    console.log("\nSkipping English translations — export files not found in public/translations/");
    console.log("Run: node scripts/export-translations.mjs to generate them.");
    return;
  }
  ...
  const versionsEn: Record<string, { nameEn: string; descriptionEn: string }> = JSON.parse(
    readFileSync(versionsFile, "utf-8"),
  );
  let vCount = 0;
  for (const [yearStr, trans] of Object.entries(versionsEn)) {
    const year = Number.parseInt(yearStr, 10);
    await db.execute(
      sql`UPDATE constitution_versions SET name_en = ${trans.nameEn}, description_en = ${trans.descriptionEn} WHERE year = ${year}`,
    );
    vCount++;
  }
  console.log(`  Applied ${vCount} version translations`);

  // 2. Structural unit translations
  const unitsEn: Record<string, string> = JSON.parse(readFileSync(unitsFile, "utf-8"));
  let uCount = 0;
  for (const [key, nameEn] of Object.entries(unitsEn)) {
    const [yearStr, _type, _numStr, slug] = key.split(":");
    const year = Number.parseInt(yearStr, 10);
    await db.execute(
      sql`UPDATE structural_units SET name_en = ${nameEn}
          WHERE slug = ${slug}
          AND version_id = (SELECT id FROM constitution_versions WHERE year = ${year})`,
    );
    uCount++;
  }
  console.log(`  Applied ${uCount} structural unit translations`);

  // 3. Article translations
  const articlesEn: Record<
    string,
    { titleEn: string | null; contentEn: string; contentTiptapEn: unknown }
  > = JSON.parse(readFileSync(articlesFile, "utf-8"));
  let aCount = 0;
  for (const [key, trans] of Object.entries(articlesEn)) {
    const [yearStr, numStr] = key.split(":");
    const year = Number.parseInt(yearStr, 10);
    const articleNum = Number.parseInt(numStr, 10);
    const tiptapJson = trans.contentTiptapEn ? JSON.stringify(trans.contentTiptapEn) : null;
    await db.execute(
      sql`UPDATE articles SET
            title_en = ${trans.titleEn},
            content_en = ${trans.contentEn},
            content_tiptap_en = ${tiptapJson}::jsonb
          WHERE number = ${articleNum}
          AND version_id = (SELECT id FROM constitution_versions WHERE year = ${year})`,
    );
    aCount++;
  }
  console.log(`  Applied ${aCount} article translations`);

  console.log("  ✅ All English translations applied");
}
