...
/**
 * Dynamic Open Graph image for each article page.
...
export default async function OgImage({
  ...
  if (!Number.isNaN(yearNum)) {
    ...
    if (version) {
      ...
      let artNum: number | null = null;
      if (slug && slug.length > 0) {
        const lastSegment = slug[slug.length - 1];
        const match = lastSegment.match(/^articolul-(\d+)$/);
        if (match) {
          artNum = Number.parseInt(match[1], 10);
        }
      }
      ...
      if (artNum !== null) {
        const [article] = await db
          ...
          .from(articles)
          .where(and(eq(articles.versionId, version.id), eq(articles.number, artNum)))
          .limit(1);
